...
class Boogle:
    ...
    def __init__(self, master):
        ...
        self.empty_frame = Frame(master, bg='black', width=200, height=600)
        self.empty_frame.grid(row=0, column=0)

        self.board_frame = Frame(master, bg='black', width=440, height=800)
        self.board_frame.grid(row=0, column=1)

        self.score_frame = Frame(master, bd=20, bg='black', width=80, height=80)
        self.score_frame.place(x=872, y=70)


        ############################################################### board frame ##############################################################################################

        self.timer_frame = Frame(self.board_frame, bd=20, bg='black', width=100, height=100)
        self.timer_frame.grid(row=0, column=0)

        self.screen_frame = Frame(self.board_frame)
        self.screen_frame.grid(row=1, column=0)

        self.letters_frame = Frame(self.board_frame, bd=20, bg='black', width=600, height=600)
        self.letters_frame.grid(row=2, column=0)

        self.empty_space = Frame(self.board_frame, bd=20, bg='black', width=440, height=30)
        self.empty_space.grid(row=3, column=0)

        self.submit_frame = Frame(self.board_frame, bd=20, bg='black', width=90, height=50)
        ...
        self.exit_button = Button(self.end_game_menu, text='Quit', font='Courier 20', command=self.quit_game)
        self.exit_button.grid(row=1, column=0, sticky=N)
        self.submit_button = Button(self.submit_frame, text='Submit', font='Courier 20', command=self.end_of_word)
        self.submit_button.place(x=-15, y=-15)
        # initiates the board
        for col in range(BOARD_SIZE):
            for row in range(BOARD_SIZE):
                button = Button(self.letters_frame, text=self.board_dict[(col, row)], fg='red', font='Courier', width=9,
